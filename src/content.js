...
function getCurrentCodeAndProblem() {
  try {
    console.log("Extracting code and problem data...");
    ...
    // Get problem description with more selectors
    ...
    for (const selector of languageSelectors) {
      const element = document.querySelector(selector);
      if (element && element.textContent.trim()) {
        language = element.textContent.toLowerCase().trim();
        break;
      }
    }
    ...
    console.log("Final extracted data:", {
      ...
      language,
      codePreview: userCode.substring(0, 200) + "...",
    });

    return {
      problemTitle,
      problemDescription: problemDescription.substring(0, 500),
      userCode,
      language,
    };
  } catch (error) {
